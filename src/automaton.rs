...
impl Automaton {
    ...
    pub fn event(&mut self, event: &quicksilver::lifecycle::Event) {
        self.state = self.state.event(&mut self.board_state, GameEvent::wrap_io(*event));
    }

    /// call periodically so current state can measure elapsed time
    pub fn update(&mut self) {
        self.state = self.state.update(&mut self.board_state);
    }
    ...
    pub fn draw(&self, window: &mut Window) -> Result<()> {
        ...
        self.state.draw(&self.board_state, window, 1);
        Ok(())
    }
}